...
sub simulate_reads{
	...
	for my $x( 1 .. $read_num ){
		...
		if($if_1st == 1){
			...
			print OUT "**\t1\n" if ($start_loci2%$seq_length >= $seq_length-$read_length+5 and $start_loci2%$seq_length <= $seq_length-5);
			print OUT "**\t2\n" if ($start_loci >= $seq_length-$read_length+5 and $start_loci <= $seq_length-5);
			$bsj ++ if($start_loci2%$seq_length >= $seq_length-$read_length+5 and $start_loci2%$seq_length <= $seq_length-5 || $start_loci >= $seq_length-$read_length+5 and $start_loci <= $seq_length-5);
		}
	} print BSJ "$bsj\n";
}
