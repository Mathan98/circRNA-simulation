use 5.012;
use Getopt::Long;
my ($circ_fq1, $circ_fq2, $out, $gtf, $coverage, $coverage2, $rand_mode, $rand_mode2, $read_length, $seq_err, $insert_length, $insert_length2, $perc_minor, $sigma, $sigma2, $ref_dir, $read_number, $help, $if_chr, $exon_skipping, $psi);
...
my @die_reason;
my $if_die;
#my ($circ_fq1, $circ_fq2, $gtf) = (">>./simulate_80_10X_80bp_200_1.fq",">>./simulate_80_10X_80bp_200_2.fq",">>./gtf_80_10X_80bp_200.out");
#my $coverage = 10;
#my $coverage2 = 100;
#my $rand_mode = 1;
#my $rand_mode2 = 2;
...
if(defined($help)){
	...
	print "\t-RN\t\tdesignated read numbers or names\n";
...
for my $chromo(@chr){
	...
	GEN: for my $gene(keys %{$chr_gene_trsc_exon{$chromo}}){
		...
		my $trsc = $trscs[$trsc_rand];
			...
			my $trsc_str = $chr_gene_trsc_exon{$chromo}{$gene}{$trsc}[0][0];
			my $trsc_end = $chr_gene_trsc_exon{$chromo}{$gene}{$trsc}[$#{$chr_gene_trsc_exon{$chromo}{$gene}{$trsc}}][1];
...
sub simulate_reads2{
	...
	my $read_num = int( $seq_length * $trsc_coverage / $read_length / 2 );
